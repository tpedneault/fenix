...
fn parse_line(line: &str) -> Option<Branch> {
    ...
    let (ahead, behind) = parse_track(fields.next().unwrap_or(""));
    Some(Branch { name, current, upstream, ahead, behind })
}
...
mod tests {
    ...
    fn parse_line_reads_the_current_branch_with_no_upstream() {
        ...
        assert_eq!(branch, Branch { name: "main".to_string(), current: true, upstream: None, ahead: 0, behind: 0 });
    }
