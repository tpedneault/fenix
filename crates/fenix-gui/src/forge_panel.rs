...
pub fn render_list(project: &str, filter: MrFilter, requests: &[MergeRequest], error: Option<&str>) -> GitPanel {
    ...
    for mr in requests {
        ...
        b.push(
            &format!("{badge}{}", draft_prefixed(mr)),
            GitLineStyle::Commit,
...
pub fn render_detail(
    ...
    if !mr.description.trim().is_empty() {
        ...
        for source_line in mr.description.trim().lines() {
            ...
            for line in crate::wrap::wrap_text(source_line, crate::wrap::DEFAULT_WRAP_WIDTH) {
                b.detail(&format!("    {line}"));
            }
        }
    }
    ...
    for file in files {
        ...
        b.push(
            &format!("{badge}{}", file.display_path()),
            GitLineStyle::File,
            Some(GitEntry::File(file.display_path().to_string())),
            Some((badge.chars().count(), badge_for(file))),
        );
    }
    b.finish()
}
...
impl Builder {
    ...
    fn field(&mut self, label: &str, value: &str) {
        ...
        let width = crate::wrap::DEFAULT_WRAP_WIDTH.saturating_sub(dim_from).max(20);
...
mod tests {
    use super::*;
    use fenix_forge::{DiffRefs, FileChange, PipelineStatus};
    ...
    fn a_draft_is_not_labelled_twice() {
        ...
        already.draft = true;
        let panel = render_list("g/p", MrFilter::AllOpen, &[already], None);
...
{}", panel.text);
    ...
    fn a_draft_whose_title_does_not_say_so_still_gets_the_label() {
        ...
        plain.draft = true;
        let panel = render_list("g/p", MrFilter::AllOpen, &[plain], None);
...
{}", panel.text);
    ...
    fn every_rendered_row_is_exactly_one_line_of_metadata() {
        ...
        let panel = render_detail(Some(&multi), None, &[], None);
        assert_eq!(
...
{}", panel.text);
    ...
    fn a_list_row_count_matches_its_metadata_too() {
        let panel = render_list("g/p", MrFilter::AllOpen, &[mr(1, "a"), mr(2, "b")], None);
    ...
    fn the_list_leads_each_row_with_the_number_people_actually_say() {
        let panel = render_list("group/project", MrFilter::AllOpen, &[mr(42, "Add the thing")], None);
    ...
    fn a_blocked_request_gets_a_bad_badge_so_a_scan_finds_it() {
        ...
        let panel = render_list("g/p", MrFilter::AllOpen, &[failing, conflicted, draft, mr(4, "Ready")], None);
    ...
    fn a_row_carries_its_number_so_the_action_keys_can_find_it() {
        let panel = render_list("g/p", MrFilter::Mine, &[mr(42, "t")], None);
    ...
    fn an_unreachable_instance_says_so_in_the_pane_not_just_the_modeline() {
        ...
        let panel = render_list("g/p", MrFilter::Mine, &[], Some("HTTP 401 (Unauthorized)"));
    ...
    fn the_detail_pane_answers_whether_this_can_merge() {
        ...
        let panel = render_detail(Some(&mr(42, "Add the thing")), Some(&approvals), &[file("src/a.rs", FileChange::Modified)], None);
        assert!(panel.text.contains("!42 Add the thing"), "got:\n{}", panel.text);
    ...
    fn a_conflicted_request_says_so_where_it_cannot_be_missed() {
        ...
        conflicted.has_conflicts = true;
        let panel = render_detail(Some(&conflicted), None, &[], None);
    ...
    fn an_instance_with_no_approval_rules_reports_approvers_not_a_rule() {
        ...
        let panel = render_detail(Some(&mr(1, "t")), Some(&approvals), &[], None);
    ...
    fn a_long_description_is_wrapped_rather_than_running_off_the_pane() {
        ...
        let panel = render_detail(Some(&long), None, &[], None);
        assert!(
    ...
    fn nothing_selected_says_what_to_do_rather_than_rendering_blank() {
        let panel = render_detail(None, None, &[], None);
