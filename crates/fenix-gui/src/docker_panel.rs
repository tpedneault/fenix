...
pub enum DockerLineStyle {
    ...
    Image,
    Volume,
    /// A `label: value` row in the Details pane -- no `entry` (nothing
    /// to act on), just a dimmable label prefix like `Container`/`Image`
    /// rows already have.
    Detail,
    Footer,
    /// Shown when a list comes back empty -- no `docker` on `PATH`, an
    /// unreachable daemon, or genuinely nothing to show (also reused
    /// for the Details pane's "nothing selected" placeholder).
    Empty,
}

/// What the Details pane is currently describing -- one of the three
...
/// The Containers pane's own content (`SPC d d`'s multi-pane layout)
/// -- no in-buffer "Containers" header line, since the pane's own
/// title bar (see `app.rs`'s `pane_titles`) already says so; a real
/// header line there would just be redundant. `stats`, when non-empty,
/// gets merged into
/// each row's dim suffix by a tolerant id match (`find_stat`) -- pass
/// `&[]` before the first stats tick has landed.
pub fn render_containers(containers: &[Container], stats: &[ContainerStat]) -> DockerPanel {
    let mut b = Builder::new();
    if containers.is_empty() {
        b.push("    No containers found", Some(DockerLine { style: DockerLineStyle::Empty, entry: None, dim_from: None }));
    } else {
        for c in containers {
            let prefix = format!("  {}  ", c.name);
            let dim_from = prefix.chars().count();
            let stat_suffix = find_stat(stats, &c.id).map(|s| format!("  CPU: {}  MEM: {}", s.cpu_percent, s.mem_usage)).unwrap_or_default();
            let line = format!("{prefix}{}  {}{stat_suffix}", c.image, c.status);
            b.push(
                &line,
                Some(DockerLine {
    ...
    b.blank();
    b.push(
        "  s start  S stop  R restart  x remove  l logs  u refresh",
...
/// returns them at different lengths.
fn find_stat<'a>(stats: &'a [ContainerStat], id: &str) -> Option<&'a ContainerStat> {
    stats.iter().find(|s| s.container_id == id || id.starts_with(&s.container_id) || s.container_id.starts_with(id))
}

/// The Images pane's own content -- same "no in-buffer header, the
/// pane title bar already has one" reasoning as `render_containers`.
pub fn render_images(images: &[Image]) -> DockerPanel {
    let mut b = Builder::new();
    if images.is_empty() {
        b.push("    No images found", Some(DockerLine { style: DockerLineStyle::Empty, entry: None, dim_from: None }));
    } else {
        for img in images {
    ...
    b.blank();
    b.push("  r run  x remove  u refresh", Some(DockerLine { style: DockerLineStyle::Footer, entry: None, dim_from: None }));
    b.finish()
}
...
pub fn render_volumes(volumes: &[Volume]) -> DockerPanel {
    let mut b = Builder::new();
    if volumes.is_empty() {
        b.push("    No volumes found", Some(DockerLine { style: DockerLineStyle::Empty, entry: None, dim_from: None }));
    } else {
        for v in volumes {
    ...
    b.blank();
    b.push("  u refresh", Some(DockerLine { style: DockerLineStyle::Footer, entry: None, dim_from: None }));
    b.finish()
}

/// The Details pane's own content: plain `label: value` rows from
/// whichever resource is currently selected in the focused left pane
/// (`None` while nothing's been navigated to yet, or the Details pane
/// itself is focused) -- pure formatting of fields `fenix_docker`
/// already produces, no `docker inspect` call (see the plan's Scope
/// for why that's deliberately out for this pass).
pub fn render_details(detail: Option<&DockerDetail>) -> DockerPanel {
    let mut b = Builder::new();
    match detail {
        None => {
            b.push("    Nothing selected", Some(DockerLine { style: DockerLineStyle::Empty, entry: None, dim_from: None }));
        }
...
fn push_detail_line(b: &mut Builder, label: &str, value: &str) {
    ...
    b.push(
        &format!("{prefix}{value}"),
        Some(DockerLine { style: DockerLineStyle::Detail, entry: None, dim_from: Some(dim_from) }),
    );
}
...
mod tests {
    ...
    #[test]
    fn render_containers_merges_a_matching_stat_into_the_row() {
        let panel = render_containers(&[container("abc123", "web")], &[stat("abc123")]);
        assert!(panel.text.contains("CPU: 1.23%"));
        assert!(panel.text.contains("MEM: 10MiB / 2GiB"));
    }

    #[test]
    fn render_containers_with_no_matching_stat_shows_no_stat_suffix() {
        let panel = render_containers(&[container("abc123", "web")], &[stat("other999")]);
    ...
    #[test]
    fn render_volumes_footer_has_no_destructive_key() {
        let panel = render_volumes(&[volume("myvol")]);
        assert!(panel.text.contains("u refresh"));
        assert!(!panel.text.contains("x remove"));
    }

    #[test]
    fn render_details_none_shows_nothing_selected() {
        let panel = render_details(None);
        assert!(panel.text.contains("Nothing selected"));
    }

    #[test]
    fn render_details_container_shows_its_fields() {
        let panel = render_details(Some(&DockerDetail::Container(container("id1", "web"))));
    ...
    fn render_details_image_shows_its_fields() {
        let panel = render_details(Some(&DockerDetail::Image(image("id1", "nginx", "latest"))));
    ...
    fn render_details_volume_shows_its_fields() {
        let panel = render_details(Some(&DockerDetail::Volume(volume("myvol"))));
