...
impl BufferKind {
    ...
    /// Whether a buffer of this kind belongs to a live, in-memory
    /// session (`GitSession`, `DockerSession`, `DebugSession`, ...) that
    /// nothing on the session-restore path can rebuild.
    ///
    /// Such a buffer is never written to the session file as a
    /// `Document` (it has no path, and its text is regenerated by the
    /// host anyway), so a pane holding one restores pointed at a shared
    /// placeholder instead. For a whole multi-pane panel that's useless:
    /// the host restores the Git dashboard's seven-pane shape with all
    /// seven panes showing the same placeholder. `fenix-gui`'s session
    /// layer uses this to skip saving such a workspace at all, so
    /// reopening the panel builds a real one.
    ///
    /// Deliberately an exhaustive `match` and not a `matches!` list:
    /// this started out as the latter, and `Diff` -- the Git panel's own
    /// Main pane -- was simply missing from it, which silently defaulted
    /// that whole workspace to "restorable" and produced exactly the
    /// seven-placeholder bug above. Adding a variant should be a compile
    /// error here until someone decides which side it belongs on.
    pub fn is_session_owned(self) -> bool {
        match self {
            // Real files, or a real file retagged in place.
            BufferKind::Text | BufferKind::Table => false,
            // Regenerated from scratch on restore, and meaningful on
            // their own -- the start screen is literally what a restored
...
mod tests {
    ...
    /// The panes a multi-pane panel is actually built from. `Diff` is
    /// the one that matters most here: the Git dashboard's Main pane is
    /// a `Diff` buffer, not a `Git` one, and when it was missing from
    /// this classification the whole Git workspace looked restorable
    /// and came back as seven identical placeholder panes.
    #[test]
    fn every_kind_a_panel_pane_can_hold_is_session_owned() {
        for kind in [
            BufferKind::Git,
            BufferKind::Diff,
            BufferKind::Graph,
            ...
            assert!(kind.is_session_owned(), "{kind:?} lives in a panel workspace and can't be rebuilt on restore");
        }
    }

    /// The other side: a real file must never be mistaken for panel
    /// scaffolding, or restoring a session would quietly drop the
    /// workspace the user's actual work was in.
    #[test]
    fn a_real_file_is_never_session_owned() {
        assert!(!BufferKind::Text.is_session_owned());
        assert!(!BufferKind::Table.is_session_owned());
        assert!(!BufferKind::Dashboard.is_session_owned());
        assert!(!BufferKind::Explorer.is_session_owned());
    }
