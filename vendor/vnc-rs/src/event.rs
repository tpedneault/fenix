...
pub enum VncEvent {
    ...
    /// One server `FramebufferUpdate` message has been fully decoded --
    /// every `RawImage`/`Copy`/... event emitted since the previous
    /// `FramebufferUpdateEnd` (or since the connection opened) belongs to
    /// that one message, and the framebuffer is only guaranteed to be
    /// self-consistent once all of them have been applied.
    ///
    /// RFB's atomic unit is the message, not the rectangle: a
    /// `FramebufferUpdate` header carries a `number-of-rectangles` count,
    /// and `CopyRect` in particular is defined against the framebuffer as
    /// it was at the *start* of the update. A client that paints partway
    /// through one shows genuinely inconsistent frames (content visible
    /// in two places at once, not-yet-erased regions). Without this
    /// event, a consumer of the per-rectangle stream has no way to know
    /// where one update ends and the next begins, so it cannot present
    /// whole frames or implement the usual one-outstanding-request flow
    /// control.
    ///
    /// Emitted for both ways an update terminates: the rectangle count
    /// being exhausted, and a [crate::VncEncoding::LastRectPseudo]
    /// rectangle ending it early.
