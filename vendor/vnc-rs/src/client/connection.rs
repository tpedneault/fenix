    ...
    while let Err(oneshot::error::TryRecvError::Empty) = stop_ch.try_recv() {
        ...
        match server_msg {
            ServerMsg::FramebufferUpdate(rect_num) => {
                ...
                // Local patch (see `vendor/vnc-rs/PATCH.md`): tell the
                // consumer this whole message has landed. Reached by both
                // exits above -- the `rect_num` count running out and the
                // `LastRectPseudo` `break` -- so every update is closed
                // exactly once.
